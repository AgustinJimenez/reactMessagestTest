...
describe('HomePage', () => {
  ...
  it('should render page', async () => {
    ...
    for (let msgsList of msgsLists) {
      ...
      const messagesListTitle = await within(msgsList).findByTestId('messages-list-title')
      expect(messagesListTitle).toBeInTheDocument()
      expect(messagesListTitle).toBeVisible()

      const messagesListCount = await within(msgsList).findByTestId('messages-list-count')
      expect(messagesListCount).toBeInTheDocument()
      expect(messagesListCount).toBeVisible()

      const messagesListCountValue = await within(messagesListCount).findByTestId('messages-list-count-value')
  ...
  it('should generate and render messages', async () => {
    render(<App />)
    const numberMsgsBefore = (await screen.findAllByTestId('messages-list-item')).length
    await waitFor(
      async () => {
        const messagesListItems = await screen.findAllByTestId('messages-list-item')
        for (let messagesListItem of messagesListItems) {
          const message = await within(messagesListItem).findByTestId('messages-list-message')
          expect(message).toBeVisible()
          expect(message).toBeInTheDocument()
          expect(message).not.toBeEmptyDOMElement()
          const clearButton = await within(messagesListItem).findByTestId('messages-list-clear-button')
          expect(clearButton).toBeVisible()
          expect(clearButton).toBeInTheDocument()
        }
        const numberMsgsAfter = (await screen.findAllByTestId('messages-list-item')).length
  ...
  it('should remove a message', async () => {
    ...
    await waitFor(
      async () => {
        ...
        await within(messagesListItem).getByTestId('messages-list-clear-button').click()
  ...
  it('should remove all messages', async () => {
    ...
    await waitFor(
      async () => {
        ...
        for (let messagesListItem of messagesListItems) expect(messagesListItem).not.toBeInTheDocument()
      },
      { timeout: 5000 },
    )
  })
  ...
  it('should stop messages generation', async () => {
    render(<App />)
    const startStopButton = await screen.findByTestId('start-stop-button')
    let numberMsgsBefore = (await screen.findAllByTestId('messages-list-item')).length
    await waitFor(
      async () => {
        expect((await screen.findAllByTestId('messages-list-item')).length).toBeGreaterThan(numberMsgsBefore)
      },
      { timeout: 5000 },
    )
    ...
    numberMsgsBefore = (await screen.findAllByTestId('messages-list-item')).length
    await waitFor(
      async () => {
        await sleep(3000)
        expect((await screen.findAllByTestId('messages-list-item')).length).toBe(numberMsgsBefore)
      },
      { timeout: 4000 },
    )
  })
