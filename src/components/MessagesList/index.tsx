...
const MessagesListItem = ({ message, id }: { message: string, id?: string }) => {
  ...
  return (
    <Grid container justifyContent='space-between'>
      ...
      <ClearButtonContainerGrid container item xs={12} justifyContent='flex-end'>
        <Button onClick={removeMessage} variant='text' color='inherit' data-testid='messages-list-clear-button'>
...
const MessagesList = ({ title, count = 0, messages, type }: MessagesListProps) => {
  return (
    <Grid container data-testid='messages-list'>
      ...
      <ListContainer>
        {messages?.map?.(({ message, id }: Message, key: number) => (
          <MessagesListItemContainer key={key} type={type} data-testid='messages-list-item'>
